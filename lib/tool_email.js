...
export function composeShopifyEmail({ firstName, handle, result, link, applyUrl, postalAddress }) {
  ...
  const blocks = [
    { p: `Here is the 60-second Teardown you ran on ${what}, restated from the numbers you typed. Every dollar is arithmetic on a published rate — Shopify Payments' plan rates and USPS Ground Advantage commercial prices by zone — and your orders a month are your own figure, not an estimate.` },
    { li },
    { button: "Open your result", url: link },
    { p: "That is what a public product page can tell us. Your own exports tell us the rest: true net margin per product after payments, labels, apps and 3PL charges with ads honestly allocated, Meta and Google bleed by ad set in dollars, your demand curve per product, and whether that compare-at is a promotion or a policy. The full Profit Teardown reads them and is back in 24 hours, free." },
    { button: "Get the full Teardown", url: applyUrl },
    { p: "Your private upload page for it follows from me in a separate email within about an hour. Reply to this one if you would rather talk first." },
    { small: `You asked for this email on hubricon.com/teardown${postalAddress ? `. Hubricon · ${postalAddress}` : ""}. Nothing else follows unless you reply.` },
  ];
  const greeting = firstName ? `Hi ${String(firstName).trim().slice(0, 80)},` : "Hello,";
  const msg = { greeting, blocks };
  return { subject: `Your 60-second Teardown: ${short}`, text: renderText(msg), html: renderHtml(msg) };
